...
public class AccountHandler(IHttpClientFactory httpClientFactory) : IAccountHandler
{
    ...
    public async Task<Response<string>> RegisterAsync(RegisterRequest request)
    {
        ...
        return result.IsSuccessStatusCode
            ? new Response<string>("", 201, "Cadastro realizado com sucesso")
            : new Response<string>("", 400, "Não foi possível realizar o cadastro");
    }

    public async Task LogoutAsync()
    {
        var emptContent = new StringContent("{}", Encoding.UTF8, "application/json");
        await _client.PostAsJsonAsync("v1/identity/logout", emptContent);
    }
}